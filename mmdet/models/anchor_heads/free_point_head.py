    ...
    def loss(self,
        ...
        def match_collecting(
            ...
            sparse_indices = torch.stack([
                torch.arange(len(match_result.gt_labels)).type_as(match_result.bbox_inds)[:, None].expand_as(
        ...
        loss_match, pos_inds = self.loss_pos(cls_scores, bbox_preds, gt_bboxes, gt_labels, points, matched_inds,
                                             sparse_indices)

        neg_scores = torch.cat([
            cls_score[
                torch.sparse_coo_tensor(pos_ind, torch.ones_like(pos_ind[0]), size=cls_score.size()).to_dense() == 0]
