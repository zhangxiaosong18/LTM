...
from mmdet.core import force_fp32, bbox2delta, delta2bbox, bbox_overlaps, multi_apply, build_matcher
...
class FreeAnchorHead(AnchorHead):
    def __init__(self,
                 ...
                 loss_pos=dict(
                     type='FreeAnchorLoss', loss_weight=1.0),
                 loss_neg=dict(
                     type='FocalLoss', use_sigmoid=True, gamma=2.0, alpha=0.0, loss_weight=1.0),
                 **kwargs):
    ...
    def loss(self,
        ...
        def match_collecting(
            ...
            sparse_indices = torch.stack([
                torch.arange(len(match_result.gt_labels)).type_as(match_result.bbox_inds)[:, None].expand_as(match_result.bbox_inds),
                match_result.bbox_inds,
                match_result.gt_labels[:, None].expand_as(match_result.bbox_inds)
            ], dim=0).view(3, -1)
        ...
        loss_match, pos_inds = self.loss_pos(cls_scores, bbox_preds, gt_bboxes, gt_labels, anchors, matched_inds, sparse_indices)
        ...
        loss_focal = self.loss_neg(neg_scores, torch.zeros_like(neg_scores, dtype=torch.long), avg_factor=num_positives)
