...
#         object_box_prob = matched_box_overlap
        ...
        #
        # max_box_prob = torch.sparse.max(object_cls_box_prob, dim=0).t()
        #
        # """
        # # start
        # nonzero_indices = torch.sparse.sum(sparse_object_box_prob, dim=0).indices()
        #
        # if nonzero_indices.numel() == 0:
        #     max_box_prob = torch.zeros(cls_scores.shape[0], cls_scores.shape[1]).type_as(object_box_prob)
        # else:
        #     nonzero_max_box_prob = torch.where(
        #         (gt_labels.unsqueeze(dim=-1) == nonzero_indices[1]),
        #         torch.sparse.sum(sparse_object_box_prob, dim=2).to_dense()[:, nonzero_indices[0]],
        #         torch.tensor([0]).type_as(object_box_prob)
        #     ).max(dim=0).values
        #
        #     max_box_prob = torch.sparse_coo_tensor(
        #         nonzero_indices, nonzero_max_box_prob,
        #         size=(cls_scores.shape[0], cls_scores.shape[1])
        #     ).to_dense()
        # # end
